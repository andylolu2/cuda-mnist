...
#include "lib/dataset/mnist_reader_common.hpp"
...
namespace lib {
    namespace mnist {
        ...
        bool read_mnist_image_file_flat(
            Container& images, const std::string& path, std::size_t limit, std::size_t start = 0) {
        ...
        template <
            ...
            typename Image,
            typename Functor>
        void read_mnist_image_file(
            Container<Image>& images, const std::string& path, std::size_t limit, Functor func) {
            auto buffer = read_mnist_file(path, 0x803);
        ...
        void read_mnist_label_file(
            Container<Label>& labels, const std::string& path, std::size_t limit = 0) {
         ...
         */
        template <typename Container>
        bool read_mnist_label_file_flat(
            Container& labels, const std::string& path, std::size_t limit = 0) {
            auto buffer = read_mnist_file(path, 0x801);
         ...
         */
        template <typename Container>
        bool read_mnist_label_file_categorical(
            Container& labels,
        ...
        template <
            ...
            typename Image,
            typename Functor>
        Container<Image> read_training_images(
            const std::string& folder, std::size_t limit, Functor func) {
            Container<Image> images;
            read_mnist_image_file<Container, Image>(
                images, folder + "/train-images-idx3-ubyte", limit, func);
            return images;
        }
        ...
        template <
            ...
            typename Image,
            typename Functor>
        Container<Image> read_test_images(
            const std::string& folder, std::size_t limit, Functor func) {
            Container<Image> images;
            read_mnist_image_file<Container, Image>(
                images, folder + "/t10k-images-idx3-ubyte", limit, func);
            return images;
        }
        ...
        Container<Label> read_training_labels(const std::string& folder, std::size_t limit) {
            ...
            read_mnist_label_file<Container, Label>(
                labels, folder + "/train-labels-idx1-ubyte", limit);
            return labels;
        }
        ...
        Container<Label> read_test_labels(const std::string& folder, std::size_t limit) {
            Container<Label> labels;
            read_mnist_label_file<Container, Label>(
                labels, folder + "/t10k-labels-idx1-ubyte", limit);
            return labels;
        }
         ...
         * The dataset is assumed to be in a mnist subfolder
         *
         * \param training_limit The maximum number of elements to read from training set (0: no
         * limit) \param test_limit The maximum number of elements to read from test set (0: no
         * limit) \return The dataset
         */
        template <template <typename...> class Container, typename Image, typename Label = uint8_t>
        MNIST_dataset<Container, Image, Label> read_dataset_3d(
            const std::string& folder, std::size_t training_limit = 0, std::size_t test_limit = 0) {
            MNIST_dataset<Container, Image, Label> dataset;

            dataset.training_images = read_training_images<Container, Image>(
                folder, training_limit, [] { return Image(1, 28, 28); });
            dataset.training_labels =
                read_training_labels<Container, Label>(folder, training_limit);

            dataset.test_images = read_test_images<Container, Image>(
                folder, test_limit, [] { return Image(1, 28, 28); });
         ...
         */
        template <template <typename...> class Container, typename Image, typename Label = uint8_t>
        MNIST_dataset<Container, Image, Label> read_dataset_direct(
            const std::string& folder, std::size_t training_limit = 0, std::size_t test_limit = 0) {
            MNIST_dataset<Container, Image, Label> dataset;
        ...
        MNIST_dataset<Container, Image, Label> read_dataset_direct(
            std::size_t training_limit = 0, std::size_t test_limit = 0) {
            return read_dataset_direct<Container, Image, Label>(
                "mnist", training_limit, test_limit);
        }
